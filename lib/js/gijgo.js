...
gj.tree.config = {
    base: {
		...
		/** If checkboxes = true, then you can con config if you only want checkboxes on final nodes.
         * @type boolean
         * @default false
         * @example disabled <!-- tree -->
         * <div id="tree"></div>
         ...
         *         checkboxes: true,
		 *         : false,
         *         dataSource: [{"text":"Asia","children":[{"text":"China","children":[]},{"text":"Japan","children":[]},{"text":"Mongolia","children":[]}]},
								{"text":"North America","children":[{"text":"USA","children":[{"text":"California","children":[]},{"text":"Florida","children":[]}]},
								{"text":"Canada","children":[]},{"text":"Mexico","children":[]}]}]
         *     });
         * </script>
         * @example enabled <!-- tree -->
         * <div id="tree"></div>
...
gj.tree.methods = {
    ...
    init: function (jsConfig) {
		...
		// Si la seleccion de elementos es única, no tiene sentido que cascadeSelection y cascadeCheck puedan tener otro valor distinto de false
		if (jsConfig.selectionType == 'single') {
			jsConfig.cascadeSelection = false;
			jsConfig.cascadeCheck = false;
			jsConfig.onlyChildrenCheckbox = true;
		}
    ...
    select: function ($tree, $node, cascade) {
        var i, $children = $node.find('ul>li'), data = $tree.data();	
			

		
        if ($node.attr('data-selected') !== 'true' && gj.tree.events.select($tree, $node, $node.attr('data-id')) !== false) {

				if (data.selectionType == 'multiple' || (data.selectionType == 'single' && $children.length == 0)) {
					
					// solo marcamos con check los nodos si el nodo padre tiene tb check
					//if (!data.onlyChildrenCheckbox || (data.onlyChildrenCheckbox && record.children.length == 0)) {
					...
					// Comprobamos que el plugin existe y está cargado
					if (gj.tree.plugins.checkboxes) { 
						// Comprobamos si la opción de checkboex está activada
						if (data.checkboxes && $children.length > 0) { 
							
							// Si la selección es single, entonces le quitamos el check a todos los demás
							if (data.selectionType == 'single') {
								var $checkboxes = $tree.find('ul>li');
								for (i = 0; i < $checkboxes.length; i++) {
									gj.tree.plugins.checkboxes.private.update($tree, $($checkboxes[i]), 'checked');
								}
							}						
						}						
							// Marcamos el nodo seleccionado
							gj.tree.plugins.checkboxes.private.update($tree, $node, 'checked');							
		...
		// Expand or collapse node by clicking on the name
		$expander = $node.find('>[data-role="wrapper"]>[data-role="expander"]');
		var mode = $expander.attr('data-mode');
		...
		if (mode == 'close') {
			...
			gj.tree.methods.expand($tree, $node, cascade);
		} else if (mode == 'open') {		
			// Collapse all nodes, included the child nodes
			gj.tree.methods.collapse($tree, $node, true);			
		}		
        ...
        if ($node.attr('data-selected') === 'true' && gj.tree.events.unselect($tree, $node, $node.attr('data-id')) !== false) {
            ...
            if (cascade && $children.length > 0) {
				...
				if (gj.tree.plugins.checkboxes) { 
					...
					// Marcamos el nodo NO seleccionado
					gj.tree.plugins.checkboxes.private.update($tree, $node, 'unchecked');							
    ...
    private: {
				...
				if (data.checkboxes) {
					if (!data.onlyChildrenCheckbox || (data.onlyChildrenCheckbox && record.children.length == 0)) {
						...
						$checkbox.on('click', function (e) {
							...
							// Vamos con la parte de seleccionar nodos. Si haces click en un nodo que tiene hijos:
							// Si hay seleccion multiple, puedes hacer click en ese nodo si hay otros seleccionados
							// Si hay selección single, se elimina primero cualquier check de otros nodos antes de hacer check en este nodo							
							if (state == "checked"){
								if (data.selectionType == 'multiple' || (data.selectionType == 'single' && $children.length == 0)) {
									if (data.selectionType == 'single') {
										gj.tree.methods.unselectAll($tree);										
									...
									gj.tree.methods.select($tree, $($node), true);
								}								
								
							}
							else if (state = "unchecked"){
								if (data.selectionType == 'multiple' || (data.selectionType == 'single' && $children.length == 0)) {
									gj.tree.methods.unselect($tree, $($node), true);									
								}	
							}
							
							
							// Cuando se hace click en el checkbox puede que estemos quitando el check o poniéndolo
							// En función de eso y del estado del nodo se determina si hay que expandir o contraer el nodo
							$children = $node.find('ul>li')
							state = $node.find('[data-role="checkbox"] input[type="checkbox"]').checkbox('state');
							...
							if (mode == 'close' && state == 'checked') {
								if ($children.length > 0) {
									$expander.attr('data-mode', 'open');
									$expander.empty().append(data.icons.collapse);	
									gj.tree.methods.expand($tree, $node, true);
								}					
								
							} else if (mode == 'open' && state == 'unchecked') {		
								if ($children.length > 0) {
									$expander.attr('data-mode', 'close');
									$expander.empty().append(data.icons.collapse);	
									gj.tree.methods.expand($tree, $node, false);
								}		
							}
		...
		getRecord: function (id, records) {
			...
			for (i = 0; i < records.length; i++) {
				...
				if (children.length > 0) {
					gj.tree.plugins.checkboxes.private.getRecord(id, children);	
				}
			}
		},		
    },
